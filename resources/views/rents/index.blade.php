...
@section('content')
    ...
    <table id="rents-table" class="table table-striped" style="width:100%">
        <thead>
            <tr>
                <th>Tenant</th>
                <th>Amount Due</th>
        ...
        <tbody>
            @foreach ($rents as $rent)
                <tr>
                    ...
                    <td>
                        @if ($rent->bill)
                            PKR {{ number_format($rent->bill->amount, 2) }} on {{ \Carbon\Carbon::parse($rent->bill->date)->format('D d M y') }}
                        @else
                            N/A
                        @endif
                    </td>
