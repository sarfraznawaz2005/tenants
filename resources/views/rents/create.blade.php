        ...
        <div class="card-body">
            <form action="{{ route('rents.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('tenant_id') is-invalid @enderror" id="tenant_id" name="tenant_id" required>
                        ...
                        @foreach ($tenants as $tenant)
                            <option value="{{ $tenant->id }}" data-rent="{{ $tenant->monthly_rent }}" {{ old('tenant_id') == $tenant->id ? 'selected' : '' }}>{{ $tenant->name }}</option>
                        @endforeach
                    </select>
                    @error('tenant_id')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('bill_id') is-invalid @enderror" id="bill_id" name="bill_id">
                        ...
                        @foreach ($bills as $bill)
                            <option value="{{ $bill->id }}" data-picture="{{ $bill->picture ? asset('storage/' . $bill->picture) : '' }}" {{ old('bill_id') == $bill->id ? 'selected' : '' }}>PKR {{ $bill->amount }} on {{ \Carbon\Carbon::parse($bill->date)->format('D d M y') }}</option>
                        @endforeach
                    </select>
                    @error('bill_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('amount_received') is-invalid @enderror" id="amount_received" name="amount_received" value="{{ old('amount_received') }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('date') is-invalid @enderror" id="date" name="date" value="{{ old('date') }}" required>
                    @error('date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="status" id="not_paid" value="not_paid" {{ old('status') == 'not_paid' ? 'checked' : '' }}>
                ...
                <div class="mb-3" id="comment-section" style="{{ old('status') == 'not_paid' ? '' : 'display: none;' }}">
